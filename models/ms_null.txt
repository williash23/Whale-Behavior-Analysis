 ...
 sink("ms_null.txt")
	...
	model{
		...
		#  LIKELIHOOD
			...
			#  Probabilities of observations at a given occasion given state at that same occasion.
			po[1,1] <- 1 - pT
			po[1,2] <- pT
		...
		enc[i, 1]] ~ dcat(px0[1:2])
			...
			#   Uses multinomial logit for the transition probabilities between movement mode states.
		...
		#  PRIORS
			...
			#  For each individual, store its state.
